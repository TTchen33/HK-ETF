...
- [x] 建立 ETF 变化雷达，自动比较目录版本并保留新增、下架、费率、基准、状态与规模变化。
- [x] 建立日期与币种对齐的 ETF 溢价／折价监测，并展示逐只计算链路。
...
目录差异事件、幂等去重和自动流程见 [ETF 目录变化雷达](docs/06-change-radar.md)。
净值、汇率与同日收盘价的对齐规则见 [ETF 溢价／折价监测方法](docs/07-premium-discount-methodology.md)。
